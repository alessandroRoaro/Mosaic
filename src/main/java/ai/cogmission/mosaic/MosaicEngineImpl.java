...
class MosaicEngineImpl<T> implements MosaicEngine<T> {
	...
	private void doStats(SurfacePriviledged<T> surface, long start) {
		...
		Log.d("Divider Size: " + surface.getDividerSize());
		Log.d("Integer Precision: " + surface.getUseIntegerPrecision());
		Log.d("Area: " + surface.getArea());
        ...
        Log.d("execution time: " + ((double)finish / 1000000000.0d) + "  secs.");
	}
	...
	class InputManager {
		...
		void selectElement(List<Element<T>> elems, double x, double y) {
			...
			if(e.type == ElementType.DIVIDER) {
				Log.d("Divider " + e.stringID + "'s leading: ");
				for(Divider<T> d : ((Divider<T>)e).leadingJoins) {
					Log.d("\t " + d);
				}
				Log.d("Divider " + e.stringID + "'s trailing: ");
